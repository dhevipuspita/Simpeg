...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="d-block mb-3">
            {{-- Tambah cuti manual (opsional, lihat penjelasan di bawah) --}}
                    ...
                    <form action="{{ route('perizinan.store') }}" method="POST">
                        ...
                        <div class="modal-body">
                            <div class="row">
                                        ...
                                        <select name="staffId" id="staffId" class="form-select select2" required>
                                            ...
                                            @foreach ($staff as $st)
                                                <option value="{{ $st->staffId }}">
                                                    {{ $st->name }} @if($st->nik) - {{ $st->nik }} @endif
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                {{-- Catatan: data NPA, Jenjang, Jabatan, dll diambil dari DataInduk/Staff, bukan diisi manual --}}
                ...
                <tbody class="table-border-bottom-0">
                    @foreach ($perizinan as $key => $p)
                        <tr>
                           ...
                           <td>{{ $p->dataInduk->nama ?? $p->staff->name ?? '-' }}</td>
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        @php
                                            ...
                                            $ttlText = '-';
                                            if ($p->staff) {
                                                $ttlText = $p->staff->birthPlace ?? '';
                                                if ($p->staff->birthDate) {
                                                    $ttlText .= ($ttlText ? ', ' : '') .
                                                        \Carbon\Carbon::parse($p->staff->birthDate)->locale('id_ID')->translatedFormat('d F Y');
                                                }
                                                $ttlText = $ttlText ?: '-';
                                            }
                                        @endphp
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                <p><b>Nama Lengkap:</b> {{ $p->dataInduk->nama ?? $p->staff->name ?? '-' }}</p>
                                                <p><b>NIK:</b> {{ $p->staff->nik ?? '-' }}</p>
                                                <p><b>NPA:</b> {{ $p->dataInduk->npa ?? '-' }}</p>
                                                <p><b>Tempat, Tanggal Lahir:</b> {{ $ttlText }}</p>
                                                <p><b>Alamat:</b> {{ $p->staff->alamat ?? '-' }}</p>
                                            </div>
                                            <div class="col-md-6">
                                    ...
                                    <form action="{{ route('perizinan.update', ['id' => $p->perizinanId]) }}"
                                        ...
                                        <div class="modal-body">
                                            <div class="row">
                                                ...
                                                {{-- Nama Staff (ditampilkan saja, tidak diubah) --}}
                                                <div class="col-md-6 col-sm-12">
                                                    <div class="mb-3">
                                                        <label class="form-label">Nama Staff</label>
                                                        <input type="text" class="form-control"
                                                               value="{{ $p->staff->name ?? '-' }}" disabled>
                                                    </div>
                                                </div>
